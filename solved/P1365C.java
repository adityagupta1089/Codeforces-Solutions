...
public class P1365C {
    ...
    static class Task {
        public void solve(InputReader in, PrintWriter out, boolean debug) {
            int n = in.nextInt();
            int[] as = in.nextIntArray(n), bs = in.nextIntArray(n);
            int[] idx = new int[n + 1];
            for (int i = 0; i < n; i++) {
                idx[as[i]] = i;
            }
            ...
            int mxDeltaCnt = 0;
            for (int i = 0; i < n; i++) {
                var delta = (idx[bs[i]] - i + n) % n;
                cnt[delta]++;
                if (cnt[delta] > mxDeltaCnt) {

                    mxDeltaCnt = cnt[delta];
                }
            }
            if (debug) {
                out.println(Arrays.toString(idx));
                out.println(Arrays.toString(cnt));
            }
            out.println(mxDeltaCnt);
        }
    }
